...
const CartPage: React.FC = () => {
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      const orderData: {
        ...
        shippingAddress: typeof orderForm.shippingAddress;
        billingAddress: typeof orderForm.billingAddress;
        notes: string;
        shippingCost: number;
        taxAmount: number;
        sessionId?: string;
      ...
      // Get authentication token or sessionId
      const sessionId = getSessionId();
  ...
  if (error) {
    return (
      <>
        <Header />
        <Container maxWidth="lg" sx={{ py: 4, mt: 15 ,height: '100vh' }}>
  ...
  return (
    <>
      <Header />
      <Container maxWidth="lg" sx={{ py: { xs: 2, md: 4 }, mt: 15 ,height: '100vh' }}>
      <motion.div
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6 }}
          ...
          >
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
              <Typography variant="body2">
                You&apos;re shopping as a guest. Create an account or login for faster checkout next time!
              </Typography>
